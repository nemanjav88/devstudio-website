...
export async function DetailPage({ collection, locale, slug }: { collection: EditorialCollection; locale: Locale; slug: string }) {
  ...
  return <ContentShell locale={locale} translatedPaths={result.translatedPaths} detail>
    <article className={`content-detail content-detail--${collection}`}>
      ...
      {collection === 'solutions' || collection === 'stories'
        ? <ContentGallery gallery={collection === 'stories' ? doc.gallery?.filter(item => populated(item.image)?.id !== media?.id) : doc.gallery} locale={locale} />
