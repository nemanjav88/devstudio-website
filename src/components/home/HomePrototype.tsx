...
export function HomePrototype({ cms }: { cms?: HomeCmsData }) {
  ...
  const navItems = [
    [navLabels?.solutions || 'Solutions', '#solutions'],
    [navLabels?.projects || 'Projects', '#projects'],
    [navLabels?.capabilities || 'Capabilities', '#process'],
    [navLabels?.about || 'About', '#about'],
    [navLabels?.stories || 'Stories', '#stories'],
  ]
  ...
  const processItems = home?.process?.steps?.length
    ? home.process.steps.map(step => [step.title, step.description || ''] as [string, string])
    : steps
  ...
  const contact = () => openPanel('Let’s start with your idea.', settings?.contactEmail ? `Send your project enquiry to ${settings.contactEmail}${settings.phone ? ` or call ${settings.phone}` : ''}. The full form is coming next; this prototype does not collect or send information.` : 'The project enquiry form is coming next. This design prototype does not collect or send information. Visit the current Dev Studio website to get in touch.', 'contact')
  ...
  return <div className="studio-home" ref={root}>
    ...
    <header className="site-header">
      ...
      <nav id="main-navigation" aria-label="Main navigation" className={menu ? 'navigation is-open' : 'navigation'}>
        {navItems.map(([label, href]) => <a key={label} href={href} onClick={() => setMenu(false)}>{label}</a>)}
        <button onClick={resources}>{navLabels?.resources || 'Resources'}</button><button onClick={contact}>{navLabels?.contact || 'Contact'}</button>
        <button className="header-cta" onClick={contact}>{navLabels?.startAProject || 'START A PROJECT'} <Arrow /></button>
      </nav>
    </header>
    <main id="main">
      <section className="hero section-dark" aria-labelledby="hero-title">
        ...
        <div className="hero-stage"><Visual kind="machine" hero media={heroMedia} />
          <div className="hero-copy"><p className="eyebrow">{home?.hero?.eyebrow || 'FROM IDEA TO REALITY.'}</p><h1 id="hero-title">{home?.hero?.headline ? home.hero.headline.split('\n').map((line, index) => <span key={line}>{index > 0 && <br />}{line}</span>) : <>WE BUILD<br />WHAT OTHERS<br /><em>ONLY IMAGINE.</em></>}</h1>
            <p className="hero-disciplines">{home?.hero?.supportingLine || 'Design. Electronics. Software. Manufacturing.'}</p>
            <p className="hero-description">{home?.hero?.subtext || 'From the first idea to a finished product — engineered, built and delivered by one team.'}</p>
            <div className="hero-actions"><button className="button button-yellow" onClick={contact}>{home?.hero?.primaryCtaLabel || 'Start a Project'} <Arrow /></button><a className="text-link" href="#projects">{home?.hero?.secondaryCtaLabel || 'Explore our work'} <span aria-hidden="true">↓</span></a></div>
          </div>
        </div>
      ...
      <section className="positioning section-light section-pad" id="about">
        ...
        <h2>{home?.positioning?.headline ? home.positioning.headline.split('\n').map((line, index) => <span key={line}>{index > 0 && <br />}{line}</span>) : <>ONE TEAM.<br /><span>EVERY DISCIPLINE REQUIRED</span><br />TO BUILD THE ENTIRE PRODUCT.</>}</h2>
        <div className="position-bottom"><span className="asterisk" aria-hidden="true">✳</span><p>{home?.positioning?.supportingText || 'Industrial design, electronics, software, manufacturing and installation. Different disciplines, working at the same table. We connect every part of the process to turn ambitious ideas into things that exist.'}</p><a className="text-link" href="#advantage">Meet the one-team advantage <Arrow /></a></div>
      </section>
      <section className="work-section section-dark section-pad" id="projects">
        ...
        <div className="section-heading"><h2>{home?.selectedWork?.headline || <>IDEAS.<br /><span className="muted">MADE TANGIBLE.</span></>}</h2><p>{home?.selectedWork?.intro || <>A first look at the worlds we build in.<br />Visual studies for the projects to come.</>}</p></div>
      ...
      <section className="build-section section-light section-pad" id="solutions">
        ...
        <div className="build-layout"><h2>{home?.whatWeBuild?.headline || <>COMPLEX IDEAS.<br />COMPLETE<br /><span className="muted">SOLUTIONS.</span></>}</h2><div className="capability-list">{(buildItems?.length ? buildItems : ['Interactive Systems', 'Retail Technology', 'Brand Experiences', 'Custom Products', 'Entertainment', 'Manufacturing']).map((name, i) => <a href="#process" key={name}><span className="meta">0{i + 1}</span><span>{name}</span><Arrow /></a>)}</div></div>
      </section>
      <section className="process-section section-dark section-pad" id="process">
        ...
        <div className="process-layout"><div className="process-intro"><p className="eyebrow">{home?.process?.opening || 'YOU BRING THE IDEA.'}</p><h2>{home?.process?.headline || <>WE MAKE<br />IT <em>REAL.</em></>}</h2><div className="process-drawing" aria-hidden="true"><div /><div /><div /><span>IDEA → OBJECT</span></div><p>Every decision moves the product forward.<br />Every discipline stays connected.</p></div>
        ...
        <div className="process-closing">{home?.process?.closing || 'WE DELIVER THE PRODUCT.'}<Arrow /></div>
      </section>
      <section className="advantage section-yellow section-pad" id="advantage"><div className="section-label"><span>05 / THE ONE-TEAM ADVANTAGE</span><span>LESS DISTANCE BETWEEN IDEA AND REALITY.</span></div><h2>{home?.whyDevStudio?.headline || <>NO HANDOFF<br />BETWEEN FIVE<br />COMPANIES.</>}</h2><div className="advantage-bottom"><span className="connection-mark" aria-hidden="true">↔</span><p>{home?.whyDevStudio?.body || 'One conversation that continues from concept to installation. Designers talk to engineers. Software meets hardware. Production informs the design. The knowledge stays with the team, and the team stays with the product.'}</p><a className="text-link" href="#made-here">See where it comes together <Arrow /></a></div></section>
      <section className="made-section section-dark" id="made-here"><div className="made-visual"><Visual kind="machine" media={madeMedia} /><span className="vertical-label meta">MATERIAL / MECHANICS / MAKING</span></div><div className="made-copy"><p className="eyebrow">06 / MADE HERE</p><h2>{home?.madeHere?.headline || <>DESIGNED HERE.<br />ENGINEERED HERE.<br /><em>BUILT HERE.</em></>}</h2><p>{home?.madeHere?.body || 'Close to the materials. Close to the machines. Close to every decision that makes the final product better.'}</p><div className="location"><span className="status-dot" /><span>{home?.madeHere?.location || settings?.location || 'Banja Luka, Bosnia & Herzegovina'}</span></div><span className="meta muted">INDUSTRIAL FORM STUDY / WORKSHOP MEDIA TO FOLLOW</span></div></section>
      <section className="own-section section-light section-pad" id="own-products"><div className="section-label"><span>07 / OUR OWN IDEAS, OUT IN THE WORLD</span><span>DEV STUDIO ORIGINALS</span></div><h2>{home?.ownProducts?.headline || <>WE DON’T ONLY BUILD FOR CLIENTS.<br /><span className="muted">WE BUILD OUR OWN PRODUCTS.</span></>}</h2><div className="own-layout"><div className="own-visual"><Visual kind="play" media={ownProductsMedia} /><span className="kids-wordmark">kids<span>play</span></span></div><div className="own-copy"><p className="eyebrow">IMAGINED AND DEVELOPED BY DEV STUDIO</p><h3>A little more<br />room for <em>play.</em></h3><p>{home?.ownProducts?.body || 'Kids Play is an original Dev Studio product. The same integrated thinking, applied to an idea of our own.'}</p><button className="button button-dark" onClick={() => openPanel('Kids Play', 'An original Dev Studio product. This visual explores a playful design direction; product details and real photography will follow.', 'play')}>Discover Kids Play <Arrow /></button></div></div></section>
      <section className="stories-section section-light section-pad" id="stories"><div className="section-label"><span>08 / LATEST FROM THE STUDIO</span><span>WORK IN PROGRESS. THINKING IN MOTION.</span></div><div className="section-heading"><h2>{home?.latestFromTheStudio?.headline || <>INSIDE<br />THE MAKING.</>}</h2><span className="meta muted">{home?.latestFromTheStudio?.intro || 'EDITORIAL PREVIEWS / STORIES COMING SOON'}</span></div><div className="stories-grid">{storyItems.map(([type, title, kind]) => <button className="story" key={title} onClick={() => openPanel(title, `A placeholder for a future ${type.toLowerCase()} story. Editorial content will be added when the homepage is connected to the CMS.`, kind)}><div className="story-image"><Visual kind={kind} /></div><p className="meta">{type.toUpperCase()} <span>PREVIEW</span></p><h3>{title}<Arrow /></h3></button>)}</div></section>
      <section className="final-cta section-dark section-pad" id="contact"><div className="section-label"><span>THE NEXT THING WE BUILD COULD BE YOURS.</span><span>LET’S TALK.</span></div><h2>{home?.finalCta?.headline || <>GOT AN IDEA?<br /><em>LET’S BUILD IT.</em></>}</h2><button className="button button-yellow" onClick={contact}>{home?.finalCta?.buttonLabel || 'START A PROJECT'} <Arrow /></button>{finalCtaMedia && <img className="cta-media" src={finalCtaMedia} alt="" />}<div className="cta-rule" /></section>
    </main>
