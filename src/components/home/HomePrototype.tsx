...
export function HomePrototype({ cms }: { cms: HomeCmsData }) {
  ...
  return <div className="studio-home" ref={root}>
    ...
    <main id="main">
      ...
      <section className="made-section section-dark" id="made-here"><div className="made-visual"><img className="made-simulator-image" src="/home/devstudio-crash-simulator.png" alt="" /><span className="vertical-label meta">{t("MATERIAL / MECHANICS / MAKING")}</span></div><div className="made-copy"><p className="eyebrow">{t("06 / MADE HERE")}</p><h2>{home?.madeHere?.headline || <>{t("DESIGNED HERE.")}<br />{t("ENGINEERED HERE.")}<br /><em>{t("BUILT HERE.")}</em></>}</h2><p>{home?.madeHere?.body || t("Close to the materials. Close to the machines. Close to every decision that makes the final product better.")}</p><div className="location"><span className="status-dot" /><span>{home?.madeHere?.location || settings?.location || t("Banja Luka, Bosnia & Herzegovina")}</span></div></div></section>
