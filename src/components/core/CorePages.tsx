...
export async function AboutPage({ locale }: { locale: Locale }) {
  return <ContentShell locale={locale}>
    ...
    <section className="about-split section-dark content-pad"><div className="about-copy"><p className="eyebrow">{locale === 'bhs' ? 'BUILT FOR BRANDS. READY FOR AGENCIES.' : 'BUILT FOR BRANDS. READY FOR AGENCIES.'}</p><h2>{locale === 'bhs' ? <>Od prve ideje<br /><em>do stvarne upotrebe.</em></> : <>From the first idea<br /><em>to the real world.</em></>}</h2><p>{locale === 'bhs' ? 'Radimo direktno sa brendovima i partnerima kojima treba tim sposoban da poveže strategiju, dizajn, tehnologiju i izradu. Naš doprinos je konkretan: proizvod, sistem ili iskustvo koje može živjeti izvan prezentacije.' : 'We work directly with brands and partners who need a team able to connect strategy, design, technology and making. Our contribution is tangible: a product, system or experience that can live beyond the presentation.'}</p></div><div className="about-workshop-media"><Image src="/about/devstudio-workshop-process.png" alt={locale === 'bhs' ? 'Radionica Dev Studija' : 'Dev Studio workshop'} fill sizes="(max-width: 850px) 100vw, 45vw" /></div></section>
