...
export function SiteHeader({ locale, settings, translatedPaths, onContact }: {
  locale: Locale; settings: HomeCmsData['settings']; translatedPaths?: Partial<Record<Locale, string>>; onContact?: () => void
}) {
  ...
  const items = [
    ...
    [labels?.capabilities || t('Capabilities'), '/#process'], [labels?.about || t('About'), '/#about'],
  ...
  const contact = () => { setMenu(false); onContact?.() }
  return <header className="site-header">
    ...
    <nav id="main-navigation" aria-label={t('Main navigation')} className={menu ? 'navigation is-open' : 'navigation'}>
      ...
      {onContact ? <button onClick={contact}>{labels?.contact || t('Contact')}</button> : <a href={localizedHref('/#contact', locale)}>{labels?.contact || t('Contact')}</a>}
      {onContact ? <button className="header-cta" onClick={contact}>{labels?.startAProject || t('START A PROJECT')} <span aria-hidden="true">↗</span></button>
        : <a className="header-cta" href={localizedHref('/#contact', locale)}>{labels?.startAProject || t('START A PROJECT')} <span aria-hidden="true">↗</span></a>}
