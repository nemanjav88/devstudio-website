...
Dry run reads the committed `content-import/` package, verifies its PDF/media hashes, classifications, all 13 groups, reference integrity, bilingual slugs and rich text/copy parity. The raw `content-source/` PDF is an optional local provenance check only: if present, its hash is verified; if absent in a deployment, the importer logs that it is unavailable and continues with the packaged PDF and media. It rejects decorative/duplicate uploads, orphaned media, conceptual heroes, missing bilingual disclosures and obsolete Smart POS tier copy. Rich-text headings are kept separate from their following paragraphs. It prints per-Solution media counts and the manifest SHA-256. It **does not load Payload configuration**, query existing rows, check target database availability or predict actual CREATE vs UPDATE counts. The test explicitly supplies unusable DB configuration to prove this offline boundary.
